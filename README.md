...
|------|-------------|------|---------|:--------:|
| <a name="input_description"></a> [description](#input\_description) | n/a | `string` | `"A KMS key used to encrypt data at-rest in Amazon OpenSearch Service."` | no |
...
| <a name="input_name"></a> [name](#input\_name) | The display name of the alias. The name must start with the word "alias" followed by a forward slash (alias/). | `string` | `"alias/opensearch"` | no |
| <a name="input_principals"></a> [principals](#input\_principals) | AWS Principals that can use this KMS key.  Use ["*"] to allow all principals in the current account. | `list(string)` | `[]` | no |
